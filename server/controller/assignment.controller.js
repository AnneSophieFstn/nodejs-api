...
async function createAssignment(req, res){
    try {
        ...
        if (!req.query.horaire || !req.query.date || !req.query.computer_id || !req.query.client_id) {
        ...
        if(!computer || !client){
            return res.status(404).json({message: "Ordinateur et/ou Client introuvables.."})
        }
        ...
        console.log(error)
        return res.status(500).json({message: "Erreur lors de la création de l'attribution.."})
...
async function deleteAssignment(req, res){
    const deleteAssignment = AssignmentModel.findByIdAndDelete(req.params.assignmentId);

    if(!deleteAssignment){
        res.status(400).json({message: "Cet attribution n'existe pas.."})
    }

    return res.status(200).json({message: "L'atribution a bien été supprimé.."})
}
